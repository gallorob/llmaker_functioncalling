...
def validate_level_design(use_case: int, step: int, old_level: Level, new_level: Level) -> bool:
	if use_case == 1:
		if step == 0:  # Make a room in a swamp
			...
			return 0 < len(new_level.rooms[new_level.current_room].encounter.entities['enemy']) < 4
		elif step == 2:  # Add a new room set in the middle ages
			return len(new_level.rooms) == 2 and len(new_level.corridors) == 1
		elif step == 3:  # Add an enemy and a treasure
			return len(new_level.rooms[new_level.current_room].encounter.entities['enemy']) == 1 and len(
				new_level.rooms[new_level.current_room].encounter.entities['treasure']) == 1
		...
		if step == 0:  # Create a starting room with a torch-lit atmosphere
			...
			return len(room.encounter.entities['enemy']) == 1 and room.encounter.entities['enemy'][0].hp == 50
			...
			for encounter in new_level.corridors[-1].encounters:
				if len(encounter.entities['trap']) == 1:
					if only_one_trap:
						return False
					else:
						only_one_trap = True
			return only_one_trap
			...
			room = new_level.rooms[new_level.current_room]
			return len(room.encounter.entities['enemy']) == 1 and room.encounter.entities['enemy'][0].hp == 80
		else:
			raise ValueError("Invalid step")
	elif use_case == 3:
		...
		if step == 2:  # Place a treasure chest
			room = new_level.rooms[new_level.current_room]
			return len(room.encounter.entities['treasure']) == 1
		if step == 3:  # Introduce an enemy in the collapsed bridge room: a ghostly apparition with 60 health points
			room = new_level.rooms[new_level.current_room]
			return len(room.encounter.entities['enemy']) == 1 and room.encounter.entities['enemy'][0].hp == 60
			...
			room = new_level.rooms[new_level.current_room]
			return len(room.encounter.entities['enemy']) == 1 and room.encounter.entities['enemy'][0].hp == 40
			...
			room = new_level.rooms[new_level.current_room]
			return len(room.encounter.entities['enemy']) == 1 and room.encounter.entities['enemy'][0].hp == 70
		elif step == 8:  # Connect the magical mirror room to a circular arena with a locked gate
			return len(new_level.rooms) == 5 and len(new_level.corridors) == 4
		elif step == 9:  # Add a ferocious minotaur with 100 health points in the circular arena
			room = new_level.rooms[new_level.current_room]
			return len(room.encounter.entities['enemy']) == 1 and room.encounter.entities['enemy'][0].hp == 100
		else:
			raise ValueError('Invalid step')
	elif use_case == 4:
		if step == 0:  # Create a room with a gravity-defying effect where everything floats
			...
			room = new_level.rooms[new_level.current_room]
			return len(room.encounter.entities['treasure']) == 0 and len(room.encounter.entities['enemy']) == 1
			...
			room = new_level.rooms[new_level.current_room]
			return len(room.encounter.entities['enemy']) == 4
			...
			return len(new_level.rooms) == 6 and len(new_level.corridors) == 5
		elif step == 10:  # Add a room next to the current one
			return len(new_level.rooms) == 7 and len(new_level.corridors) == 6
		else:
			raise ValueError('Invalid step')
	elif use_case == 5:
		if step == 0:  # Create 3 rooms, each connected to the next one, all set in a different European city
			...
			room = new_level.rooms[new_level.current_room]
			return len(room.encounter.entities['enemy']) == 3
			...
			room = new_level.rooms[new_level.current_room]
			return len(room.encounter.entities['enemy']) == 2
			...
			one_less_treasure = 0
			for room in new_level.rooms.values():
				one_less_treasure += 1 if len(room.encounter.entities['treasure']) > 0 else 0
			return one_less_treasure + 1 == len(new_level.rooms)
			...
			room = new_level.rooms[new_level.current_room]
			return room.encounter.entities['enemy'][0].hp == 1000 or room.encounter.entities['enemy'][1].hp == 1000
		elif step == 12:  # Make the capybara a punker, with pink spiky hair
			room_before = old_level.rooms[old_level.current_room]
			room_after = new_level.rooms[new_level.current_room]
			return room_after.encounter.entities['enemy'][-1].description != room_before.encounter.entities['enemy'][
				-1].description
		else:
			raise ValueError('Invalid step')
	else:
